# DIP-Assignment-Photoshop-Like-Application
Self Record for Assignment Digital Image Processing
<img width="840" height="626" alt="image" src="https://github.com/user-attachments/assets/3b08b393-7447-4252-b666-1cd882ba81ab" />

### Assignment 1

- Create & Loading Image
- Geometric image transformations
- Drawing Tools (Pen, Line, Shape, Paint Bucket), Color Palette
- Color Format Converter & Color Adjustment (Hue, Saturation, Brightness, Contrast, Intensity)
- Thumbnail image
- Select ROI
- Transform Tool

  <img width="435" height="319" alt="image" src="https://github.com/user-attachments/assets/8051b854-9721-4694-8fec-59f5a6d18714" />


### Assignment 2
- Sub windows
- Gridlines & Ruler
- Real time Histogram Panel
- Enhance Image Filter
  
  <img width="439" height="324" alt="image" src="https://github.com/user-attachments/assets/d0fb9f1e-65d3-443f-92b1-335b14a0ce30" />
  
  - Blur, Gaussian Blur, Motion Blur, Radial Blur
  - Sharpen, Sharpen Edge, USM
  - Add Noise, Noise Removal, Mediam
  - Diffuse, Solarize
  - Edge Enhancement, Beatify
- Undo/Redo
- Copy/Cut/Paste (Support with system clipboard)
- Edge Detection (Canny, Roberts, Sobel, Prewitt, Laplacian)
  
  <img width="469" height="354" alt="image" src="https://github.com/user-attachments/assets/9068fee8-c1c2-478f-96b6-cca7852f4867" />
  
- Thresholding (Binary, Truncate, To zero)
- Power Law Transformation
- Piecewise Linear Transformation
- Morphological Filter
- Histogram Equalization
- Layer System
  - Layer Alpha Blend
  - Layer Mode Blend
  - Layer Clipping Mask
  - Layer Thumbnail image

### Mini Project
- Barcode Reader
  
  <img width="433" height="321" alt="image" src="https://github.com/user-attachments/assets/1e19cac3-0a67-4ed0-b827-b41c34877578" />

